...
class NegMarginalLogLikelihood_phi_noBackward(nn.Module):
    ...
    def forward(self, log_sigma_f, log_lengthscale, log_sigma_n, phi, y_train, sq_lambda):
        ...
        n=y_train.size(0)
        logQ = ( (n-m)*torch.log(sigma_n.pow(2))
                + torch.sum(torch.log( c.diag() )).mul(2)
...
class LOOCrossValidation_phi_noBackward(nn.Module):
    def __init__(self):
        super(LOOCrossValidation_phi_noBackward, self).__init__()
    ...
    def forward(self, log_sigma_f, log_lengthscale, log_sigma_n, phi, y_train, sq_lambda):
        ...
        # print(sigma_f)
        # print(lambda_diag)
        Ka = phi.mm(lambda_diag.diag().mm(phi.t())) + torch.eye(n).mul(sigma_n.pow(2))  # approx K
        # print(sigma_n)
        # print(sigma_f)
        # print(Ka)
        if torch.isnan(Ka).any().item()==1:
            return torch.from_numpy(np.array([np.nan])).float()
        if torch.isinf(Ka).any().item()==1:
            return torch.from_numpy(np.array([np.inf])).float()

        Ka, c = choleskZ(Ka) # numerical tweaks might be needed

        KaInv_y = torch.gesv( y_train.view(n, 1), Ka)[0]

        tmp = torch.trtrs(torch.eye(n),c.t(),upper=False)[0]  # solves c^T*u = I, u = c*x
        sigmas_sq = torch.trtrs(tmp,c)[0].diag().pow(-1) # solves c*x = u and extracts the inverse diagonal elements

        # mus = y_train.sub( KaInv_y.mul(sigmas_sq) )

        return sigmas_sq.log() .add( y_train.sub( y_train.sub( KaInv_y.mul(sigmas_sq) ) ).pow(2).div(sigmas_sq) ) .sum()
