...
class LBFGS(Optimizer):
    ...
    def __init__(self, params, lr=1, history_size=10,  # params: model.parameters()
        ...
        state['old_dirs'] = []  # change in gradients
        state['old_stps'] = []  # change in iterates
        #state['yHy'] = []  # store the y_k^T*H_{k-1}*y_k values

    def _numel(self):  # returns the total number of parameter elements
        if self._numel_cache is None:
            self._numel_cache = reduce(lambda total, p: total + p.numel(), self._params, 0)  # reduce: continues operations on subsequent elements, see 'https://www.geeksforgeeks.org/reduce-in-python/'
        return self._numel_cache

    def _gather_flat_grad(self):  # return the (flattened) gradient
        views = []
    ...
    def two_loop_recursion(self, vec):  # cmf Nocedal alg. 7.4
        """
        Performs two-loop recursion on given vector to obtain Hv.

        Inputs:
            vec (tensor): 1-D tensor to apply two-loop recursion to (i.e. the gradient)
        ...
        history_size = group['history_size']  # memory limit

        state = self.state['global_state']
        old_dirs = state.get('old_dirs')    # change in iterates
        old_stps = state.get('old_stps')    # change in gradients
        H_diag = state.get('H_diag')

        # compute the product of the inverse Hessian approximation and the gradient
        num_old = len(old_dirs)

        if 'rho' not in state:  # check if rho is a field of the state structure
        ...
        for i in range(num_old):
            rho[i] = 1. / old_stps[i].dot(old_dirs[i])  # rho_i = 1/( y_i^T*s_i )

        q = vec  # gradient
        for i in range(num_old - 1, -1, -1):
            alpha[i] = old_dirs[i].dot(q) * rho[i]  # alpha_i = rho_i*s_i^T*q
            q.add_(-alpha[i], old_stps[i])  # q <- q - alpha_i*y_i

        # multiply by initial Hessian
        # r/d is the final direction
        r = torch.mul(q, H_diag)  # H_diag is scalar
        for i in range(num_old):
            beta = old_stps[i].dot(r) * rho[i]  # beta = rho_i*y_i^T*r
            r.add_(alpha[i] - beta, old_dirs[i])

        return r

    def two_loop_recursion_wills(self, vec, cw):  # cmf Nocedal alg. 7.4
        """
        2-loop recursion of the update H_k = C'*H*C + tau_k*s'*s, where
        C=(I-sig_k*y_k*s_k'), sig=1/(2*c+s'*y), tau=(c*sig*y'*H*y+1)/(c+s'*y)

        Performs two-loop recursion on given vector to obtain Hv.

        Inputs:
            vec (tensor): 1-D tensor to apply two-loop recursion to (i.e. the gradient)
            cw  (scalar): weighting parameter
        ...
        if 'sig' not in state:  # check if rho is a field of the state structure
            state['sig'] = [None] * history_size
            state['tau'] = [None] * history_size
            state['alpha'] = [None] * history_size
        sig = state['sig']
        tau = state['tau']
        alpha = state['alpha']
        ...
        q = vec  # gradient
        for i in range(num_old - 1, -1, -1):  # for i=m-1:-1:0
            alpha[i] = old_dirs[i].dot(q)  # alpha_i = s_i^T*q
            sy       = old_dirs[i].dot(old_stps[i])
            sig[i]   = 1/(2*cw+sy)
            #tau[i]   = (cw*sig[i]*yHy[i]+1)/(cw+sy)
            tau[i]   = (cw*sig[i]*torch.mul(old_stps[i],H_diag).dot(old_stps[i])+1)/(cw+sy)
            q.add_(-alpha[i]*sig[i], old_stps[i])  # q <- q - sig_i*alpha_i*y_i

        # multiply by initial Hessian
        # r/d is the final direction
        r = torch.mul(q, H_diag)
        for i in range(num_old):  # for i=0:num_old-1
            beta = old_stps[i].dot(r) * sig[i]  # beta = sig_i*y_i^T*r
            r.add_(alpha[i]*tau[i] - beta, old_dirs[i])
    ...
    def curvature_update(self, flat_grad, eps=1e-2, damping=False):
        ...
        state = self.state['global_state']
        fail = state.get('fail')  # boolean indicating if the line search fails (being set in the 'step' function)

        # check if line search failed
        if not fail:

            d = state.get('d')  # search direction
            t = state.get('t')  # step length
            old_dirs = state.get('old_dirs')  # change in iterates (s)
            old_stps = state.get('old_stps')  # change in gradients (y)
            H_diag = state.get('H_diag')
            prev_flat_grad = state.get('prev_flat_grad')  # g_{k-1}
            Bs = state.get('Bs')

            # compute y's
            y = flat_grad.sub(prev_flat_grad)  # g_k-g_{k-1}
            s = d.mul(t)  # s = x_{k+1}-x_k = x_k + t*d - x_k = t*d
            sBs = s.dot(Bs)
            ys = y.dot(s)  # y^T*s
            ...
            else:  # if ys > eps*sBs or damping == True:
                # save skip
                state['curv_skips'] += 1
                if debug:
    ...
    def _step(self, p_k, g_Ok, g_Sk=None, options={}):  # underscore (_step) : private method, see the receiving step function below
        """
        ...
        if prev_flat_grad is None:
            ...
            prev_flat_grad.copy_(g_Ok) # prev_flat_grad <- g_Ok
        ...
        if line_search: # perform Armijo backtracking line search
            ...
            if Bs is None:
                ...
                Bs.copy_(g_Sk.mul(-t))  # Bs = B*s = B*(x_k + t*d - x_k) = B*t*d = -g*t
